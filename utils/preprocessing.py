...
def _update_id(ratings, tags):
    ...
    ratings = pd.merge(ratings, updated_itemId, on='itemId')[['userId', 'new_itemId', 'rating']].rename(
        columns={'new_itemId': 'itemId'})
    ratings = pd.merge(ratings, updated_userId, on='userId')[['new_userId', 'itemId', 'rating']].rename(
        columns={'new_userId': 'userId'})

    # remove items only in tag interacitons and users only in tag interactions: before: 310,041 after: 305,437    
    tags = pd.merge(tags, updated_itemId, on='itemId')[['userId', 'new_itemId', 'tagId']].rename(
        columns={'new_itemId': 'itemId'})
    tags = pd.merge(tags, updated_userId, on='userId')[['new_userId', 'itemId', 'tagId']].rename(
        columns={'new_userId': 'userId'})
...
def preprocess(ratings, tags, items, tag_user_threshold, tag_item_threshold):
    ...
    items = pd.merge(items, updated_itemId, on='itemId')[['new_itemId', 'name']].rename(
        columns={'new_itemId': 'itemId'})
