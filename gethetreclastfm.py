...
def main(args):
    ...
    tags = tags.rename(columns={'userID': 'userId', 'artistID': 'itemId', 'tagValue': 'tag'})[
        ['userId', 'itemId', 'tag']]
    ...
    ratings, tags, interactions, artists, tag_tagId = preprocess(ratings=ratings, tags=tags,
                                                                 items=artists,
                                                                 tag_user_threshold=args.tag_user_threshold,
                                                                 tag_item_threshold=args.tag_item_threshold)
    ...
    for train_index, test_index in kf.split(interactions):
        train, valid, test = get_kfold(ratings, tags, interactions, train_index, test_index, args.val_ratio)

        train[0].sort_values(by='userId').to_csv(os.path.join(args.save_dir, 'tr_ratings' + str(k) + '.csv'),
                                                 index=False)
        train[1].sort_values(by='userId').to_csv(os.path.join(args.save_dir, 'tr_tags' + str(k) + '.csv'), index=False)
        valid[0].sort_values(by='userId').to_csv(os.path.join(args.save_dir, 'val_ratings' + str(k) + '.csv'),
                                                 index=False)
        valid[1].sort_values(by='userId').to_csv(os.path.join(args.save_dir, 'val_tags' + str(k) + '.csv'), index=False)
        test[0].sort_values(by='userId').to_csv(os.path.join(args.save_dir, 'te_ratings' + str(k) + '.csv'),
                                                index=False)
