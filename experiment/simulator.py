...
def simulator(df_UIT, item_emb, tag_emb, prec_W, prec_item, prec_tag, alpha, r_ui, s_ut, t_it, k,step, query_ranker, threshold=5, gpu=False):
    #3, 5
    result = dict()
    for n in k:
        result[n] = []
    ...
    for uit in tqdm(df_UIT.itertuples()):
        res = user_simulator(uit, item_emb, tag_emb, prec_W, prec_item, prec_tag, alpha, r_ui, s_ut, t_it, k,step,query_ranker, threshold, gpu)
        if res is not None:
            for n in k:
                result[n].append(res[n])
    return result

def user_simulator(uit, item_emb, tag_emb, prec_W, prec_item, prec_tag, alpha, r_ui, s_ut, t_it, k,step,query_ranker, threshold, gpu=False):
    result = dict()
    for n in k:
        result[n] = []
    redun_queries = []
    ...
    for _ in range(step):
        ...
        queries = select_single_question(query_rank, redun_queries)
        redun_queries.append(queries)

        user_response = get_user_reponse(queries, relevant_tags)
        X = tag_emb[queries].reshape((-1,1))
