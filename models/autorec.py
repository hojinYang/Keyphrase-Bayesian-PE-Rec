    ...
    def train_model(self, rating_matrix, epoch=100, batches=None, **unused):
        ...
        for _ in pbar:
            for step in range(len(batches)):
                ...
                l,_ = self.sess.run([self.loss, self.train], feed_dict=feed_dict)
            print(l)
...
def autorec(matrix_train, embeded_matrix=np.empty((0)), iteration=100, lam=0.001, rank=200,
            optimizer='RMSProp', seed=1, **unused):
    matrix_input = matrix_train
    if embeded_matrix.shape[0] > 0:
        matrix_input = vstack((matrix_input, embeded_matrix.T))

    _, n = matrix_input.shape
    model = AutoRec(n, rank, batch_size=300, lamb=lam, optimizer=tf.train.RMSPropOptimizer)
